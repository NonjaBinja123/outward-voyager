"""
gather_nearest_item — navigate to the closest scene object and interact with it.
Use when a nearby loot item or pickup is visible in the area scan.
"""


async def run(ctx) -> None:
    """Navigate to the nearest non-character scene object and interact with it."""
    objects = ctx.nearby_objects  # from last scan_nearby result
    pickups = [
        o for o in objects
    ...
    if x is not None and z is not None:
        ...
        await ctx.wait(4.0)
    # Interact with whatever is now in range
